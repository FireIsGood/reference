...
window.addEventListener("load", () => {
  ...
  /******** List Collapsible ***********/
  document
    .querySelectorAll("ul.collapsible > li > strong")
  ...
  document.querySelectorAll(".h3-wrap ul").forEach(function (ul, i) {
    ...
    let no_border_length =
      length === 1 || length % cols === 0 ? cols : length % cols;
  ...
  const searchEngine = function (options) {
    ...
    this.container =
      typeof settings.container != "undefined"
        ? settings.container
        : defaultOptions.container;
    this.trigger =
      typeof settings.trigger != "undefined"
        ? settings.trigger
        : defaultOptions.trigger;
    this.dbPath =
      typeof settings.dbPath != "undefined"
        ? settings.dbPath
    ...
    doSearch: async function (value, history) {
      ...
      await _this.fetchData().then(() => {
        if (
          ...
          this.lastSearch.resp !== undefined
        ) {
          ...
          let results =
            value !== ""
              ? _this.index.search(`${value}`)
              : _this.lastSearch.whole;
          if (history) {
      ...
      result.matches.forEach((m) => {
        switch (m.key) {
          ...
          case "sections.h2.title":
            result.item["sections"][m.refIndex]["h2"]["title"] = _this.mark(
              m.value,
              m.indices,
            );
            break;
          case "sections.h3.title":
            ...
            for (let i = 0; i < result.item["sections"].length; i++) {
              ...
              if (m.refIndex >= lastLength && m.refIndex < currLength) {
                result.item["sections"][i]["h3"][m.refIndex - lastLength][
                  "title"
                ] = _this.mark(m.value, m.indices);
                break;
              }
  ...
  // Start search
  Utils.externalLibrary(Utils.LIBRARY.fuse).then(() =>
